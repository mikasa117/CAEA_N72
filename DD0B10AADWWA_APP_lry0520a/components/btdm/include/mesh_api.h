...
enum mesh_event_type_t 
{
    ...
    MESH_EVT_INFO_LOADED,               //!< All information are loaded into mesh stack.
    ...
    MESH_EVT_UPDATE_VOLATILE_IND,       //!< Received volatile information update event, can be SEQ/IV updated..
...
enum mesh_feature_t 
{
    ...
    MESH_FEATURE_STATIC_SERVICE = (1<<15),  //!< PB-GATT and Proxy GATT service is available all the time
};
...
typedef struct
{
    ...
    union
    {
        ...
        mesh_update_ind_t       *update_volatile_ind;   //!< Mesh status update indication message, these status changes frequently.
 ...
 * @return  None.
 */
void mesh_init(mesh_composition_cfg_t *cfg, struct gap_ble_addr *addr, 
                    mesh_callback_func_t mesh_evt_cb,
                    uint16_t scan_interval, uint16_t scan_window, 
                    uint16_t adv_interval, uint8_t adv_nb,
                    uint16_t proxy_adv_interval, uint16_t prov_adv_interval);
...
/*********************************************************************
 * @fn      mesh_load_stored_info
 *
 * @brief   initialize mesh with stored information, such as app key, etc.
 *
 * @param   data    - stored information pointer.
 *          length  - information length.
 *          volatile_data    - stored volatile information pointer.
 *          volatile_length  - volatile information length.
 *
 * @return  None.
 */
void mesh_load_stored_info(uint8_t *data, uint32_t length, uint8_t *volatile_data, uint32_t volatile_length);
