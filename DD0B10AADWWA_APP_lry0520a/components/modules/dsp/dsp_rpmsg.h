...
#define RPMSG_SYNC_FUNC_SRAM_HEAP_INIT      RPMSG_SYNC_FUNC_MSG(RPMSG_SYNC_FUNC_TYPE_DSP, 0x000b)
#define RPMSG_SYNC_FUNC_MEM_USAGE_V2        RPMSG_SYNC_FUNC_MSG(RPMSG_SYNC_FUNC_TYPE_DSP, 0x000c)
...
struct rpmsg_sync_msg_sram_heap_init_t {
    uint32_t base_addr;
    uint32_t length;
};

struct rpmsg_sync_msg_mem_usage_v2_t {
    uint8_t type;
    uint32_t *curr_free;
    uint32_t *min_free;
};
