...
bool dsp_load_code_from_internal_flash(uint32_t address, uint32_t length)
{
    ...
    while (length >= sizeof(struct bin_header)) {
        /* get text section storage information */
        flash_read(QSPI0, address, sizeof(struct bin_header), (void *)&header);
        address += sizeof(struct bin_header);
        length -= sizeof(struct bin_header);
        
        if (length >= header.length) {
            length -= header.length;
            if (header.start >= DSP_IRAM_BASE_ADDR) {
                ptr = (void *)DSP_IRAM_2_MCU_SRAM(header.start);
            }
            else if (header.start >= DSP_DRAM_BASE_ADDR) {
                ptr = (void *)DSP_DRAM_2_MCU_SRAM(header.start);
            }
            else if (((header.start & 0xf0000000) == 0x20000000) || ((header.start & 0xf0000000) == 0x60000000)) {
                ptr = (void *)((header.start & 0x0fffffff) | 0x20000000);
            }
            else {
                ret = false;
                break;
...
bool dsp_load_rw_from_internal_flash(uint32_t address, uint32_t length)
{
    ...
    while (length >= sizeof(struct bin_header)) {
        ...
        if (length >= header.length) {
            ...
            else if (((header.start & 0xf000000) == 0x20000000) || ((header.start & 0xf000000) == 0x60000000)) {
                ptr = (void *)((header.start & 0x0fffffff) | 0x20000000);
                continue;
            }
            else {
            ...
            while (header.length) {
                ...
                _sub_length = sub_length;
                _buffer = buffer;
                flash_read(QSPI0, sub_address, sub_length, (void *)_buffer);
                while (sub_length) {
                    *ptr++ = *_buffer++;
                    sub_length -= 4;
                }
                header.length -= _sub_length;
                sub_address += _sub_length;
            }
            break;
        }
        else {
...
{
    SYSTEM->ClockCTRL.DSP_CACHE_PWR_OK = 1;
...
void dsp_deep_sleep(void)
{
    ...
    *(volatile uint32_t *)&SYSTEM->DSPClockEnable = 0x00000000;
    SYSTEM->DSPCTRL.DSP_RUNSTALL = 1;
    SYSTEM->ClockCTRL.DSP_CACHE_PWR_OK = 0;
}
