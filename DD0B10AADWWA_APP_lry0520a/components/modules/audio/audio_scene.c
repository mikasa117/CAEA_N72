                ...
                default:
                    if ((evt->scene == audio_scene) || (evt->scene == audio_tone_scene))
                    {
                        ...
                        if ((evt->type == AUDIO_SCENE_EVT_TYPE_RECV_ENCODED_FRAME)
                            || (evt->type == AUDIO_SCENE_EVT_TYPE_RECV_ENCODED_FRAME_V2)) {
...
void audio_scene_send_event(audio_scene_evt_t *evt)
{
    ...
    if(xPortIsInsideInterrupt()) {
        ...
        vTaskNotifyGiveFromISR(audio_scene_task_handle, &pxHigherPriorityTaskWoken);
...
void audio_scene_recv_encoded_data_v2(audio_scene_t *scene, bool valid, uint8_t index, const uint8_t *buffer, uint32_t length)
{
    audio_data_element_v2_t *elt;
    static uint8_t seq = 0;
        
    audio_scene_evt_recv_encoded_data_v2_t *evt;
    evt = (void *)pvPortMalloc(sizeof(audio_scene_evt_recv_encoded_data_v2_t));
    evt->evt.type = AUDIO_SCENE_EVT_TYPE_RECV_ENCODED_FRAME_V2;
    evt->evt.scene = scene;
    if (valid) {
        elt = (void *)pvPortMalloc(sizeof(audio_data_element_v2_t) + length);
        elt->valid = true;
        elt->index = index;
        elt->length = length;
        elt->offset = 0;
        ...
        elt = (void *)pvPortMalloc(sizeof(audio_data_element_v2_t));
        elt->valid = false;
        elt->index = index;
        elt->length = AUDIO_SPECIAL_LENGTH_FOR_PLC;
        elt->offset = 0;
