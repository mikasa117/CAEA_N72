...
int algorithm_launch(void *handle, int16_t *farend, int16_t *nearend, int16_t **out)
{
    ...
    if (trans_addr) {
        if ((uint32_t)*out >= DSP_DRAM_BASE_ADDR) {
        *out = (void *)DSP_DRAM_2_MCU_SRAM(*out);
    }
        else if ((uint32_t)*out>= DSP_SRAM_BASE_ADDR) {
            *out = (void *)DSP_SRAM_2_MCU_SRAM(*out);
        }
    }
...
void *ns_init(uint32_t sample_rate, uint8_t ns_level, uint32_t *frame_size)
{
    struct rpmsg_sync_msg_audio_ns_init *sync_msg;
    void *result;
    ...
    rpmsg_sync_invoke(rpmsg_get_remote_instance(), RPMSG_SYNC_FUNC_AUDIO_NS_INIT, sync_msg, (uint32_t *)&result);
...
int ns_exec(void *handle, const int16_t *mic_in, int16_t **out)
{
    struct rpmsg_sync_msg_audio_ns_exec *sync_msg;
    int result;
    bool trans_addr = false;
    ...
    if (((uint32_t)mic_in >= DSP_DRAM_MCU_BASE_ADDR) && ((uint32_t)mic_in < (DSP_DRAM_MCU_BASE_ADDR+DSP_DRAM_SIZE))) {
        mic_in = (void *)MCU_SRAM_2_DSP_DRAM(mic_in);
    }
    
    sync_msg->handle = handle;
    sync_msg->mic_in = mic_in;
    sync_msg->out = out;
    ...
    rpmsg_sync_invoke(rpmsg_get_remote_instance(), RPMSG_SYNC_FUNC_AUDIO_NS_EXEC, sync_msg, NULL);
...
{
    struct rpmsg_sync_msg_audio_ns_destroy sync_msg;
    
    sync_msg.handle = handle;
    rpmsg_sync_invoke(rpmsg_get_remote_instance(), RPMSG_SYNC_FUNC_AUDIO_NS_DESTROY, (void *)&sync_msg, NULL);
}
