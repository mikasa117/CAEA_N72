...
#define AUDIO_SCO_ALGO_ENABLE 1
...
#if (USING_AEC_RB == 1)
/* Define a macro to print mic and spk info. by uart2 
 * by chuzewei on 2026/03/17
 */
//#define UART2_PRINT
#ifdef  UART2_PRINT
int16_t sync_dump_buf[320];
#endif

bool enable_aec_rb = false; // ÊÇ·ñÆô¶¯aecËã·¨»·ÐÎ»º´æ
aec_ring_buff_t aec_rb;
#endif
...
typedef struct
{
    ...
    audio_hw_t *audio_hw_in;
    audio_hw_t *audio_hw_out;
...
static void init(audio_scene_t *scene)
{
    ...
    else if (param->audio_type == AUDIO_TYPE_SBC)
    {
        env->encoder = audio_encoder_init(param->audio_type, AUDIO_CHANNELS_MONO, param->sample_rate, 120, &param->encoder_param);
    }
    else
    {
        env->encoder = audio_encoder_init(param->audio_type, AUDIO_CHANNELS_MONO, param->sample_rate, FRAME_MAX_LENGTH_FIT_SINGLE, &param->encoder_param);
    }

    if ((param->hw_type_in == param->hw_type_out))
    {
        printf("-->1\r\n");
        env->audio_hw_in = audio_hw_create(param->hw_type_in,
                                           hw_request_sco_pcm_cb,
                                           param->hw_base_addr_in,
                                           AUDIO_HW_DIR_INOUT,
                                           param->sample_rate,
                                           AUDIO_CHANNELS_STEREO);
        env->audio_hw_out = env->audio_hw_in;
        env->audio_hw_output = audio_hw_output_add(env->audio_hw_in, hw_receive_adc_pcm_cb);
    }
    else
    {
        printf("-->2\r\n");
        ...
        env->audio_hw_out = audio_hw_create(param->hw_type_out,
                                            hw_request_sco_pcm_cb,
                                            param->hw_base_addr_out,
                                            AUDIO_HW_DIR_OUT,
                                            param->sample_rate,
                                            AUDIO_CHANNELS_MONO);
        env->audio_hw_output = audio_hw_output_add(env->audio_hw_in, hw_receive_adc_pcm_cb);
...
static void destroy(audio_scene_t *scene)
{
    ...
    if (env->audio_hw_in == env->audio_hw_out)
    {
        audio_hw_destroy(env->audio_hw_in);
    }
    else
    {
        /* release audio hardware */
        audio_hw_destroy(env->audio_hw_in);
        audio_hw_destroy(env->audio_hw_out);
    }
...
#if AUDIO_SCO_ALGO_ENABLE
                && (sco_env->decoder_output_wr_ptr >= sco_env->algo_frame_size))
            {
#else
            )
            {
                ...
                if (enable_aec_rb)
                {
                    uint16_t wr_len = aec_rb_write(&aec_rb, sco_env->decoder_output, sco_env->algo_frame_size);
                    memset(sco_env->decoder_output, 0, sco_env->algo_frame_size * sizeof(uint16_t));
                    uint16_t re_len = aec_rb_read(&aec_rb, sco_env->decoder_output, sco_env->algo_frame_size);
                }
                
                #ifdef UART2_PRINT
                // ½«spkÒôÆµºÍmicÒôÆµ×÷ÎªÁ¢ÌåÉù½øÐÐ´®¿ÚÊä³ö£¬¶Ô±ÈÁ½ÕßµÄÊ±¼ä²î
                for (int i = 0; i < 160; i++)
                {
                    // ×óÉùµÀ·Å Mic ÐÅºÅ
                    sync_dump_buf[2 * i] = sco_env->adc_input[sco_env->adc_input_rd_ptr + i];
                    // ÓÒÉùµÀ·Å SPK ²Î¿¼ÐÅºÅ
                    sync_dump_buf[2 * i + 1] = sco_env->decoder_output[i];
                }

                // Ê¹ÓÃÍ¬Ò»¸ö´®¿ÚÒ»´ÎÐÔ·¢³ö 640 ×Ö½Ú (160 * 2Í¨µÀ * 2×Ö½Ú) ´®¿Ú3M
                uart_transmit(uart2Handler(), sync_dump_buf, sizeof(int16_t) * 320);
                #endif
#endif                
                ...
                algorithm_launch(sco_env->audio_algo_handle,
                                 ...
                                 &output);
#else
                output = &sco_env->adc_input[sco_env->adc_input_rd_ptr];
#endif
