...
uint16_t test_read_buf[128];

void stereo_copy_r_to_l(int16_t *pcm, int samples)
{
    for (int i = 0; i < samples; i++)
    {
        pcm[2 * i + 0] = pcm[2 * i + 1]; // L = R
    }
}
...
static void event_handler(audio_scene_t *scene, audio_scene_evt_t *evt)
{
    ...
    case AUDIO_SCENE_EVT_TYPE_HW_IN_NEW_SAMPLES:
    {
        ...
        while (adc_new_samples)
        {
            ...
            samples = audio_hw_read_pcm(env->hw_output, test_read_buf, samples, param->channels);
            //                    voice_recognize_launch((int16_t *)env->buffer, samples);
            adc_new_samples -= samples;

            stereo_to_left(test_read_buf, samples);
            uart_transmit(uart2Handler(), test_read_buf, samples * 2);

            // uart_transmit(uart2Handler(), test_read_buf, samples * 4);

            //
            // stereo_copy_r_to_l((int16_t *)test_read_buf, samples);
            // uart_transmit(uart2Handler(), test_read_buf, samples * 4);
        }
        // printf("adc_new_samples = %d,param->channels = %d\r\n", adc_new_samples, param->channels);
        // uint32_t samples = 16;
        // samples = audio_hw_read_pcm(env->hw_output, test_read_buf, adc_new_samples, 1);
    }
    break;
    default:
        break;
