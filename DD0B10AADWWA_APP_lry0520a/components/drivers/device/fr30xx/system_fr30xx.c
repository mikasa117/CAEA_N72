 ...
 * @fn      main_entry_point
 *
 * @brief   do pre-init steps before main.
 */
__RAM_CODE void main_entry_point(void)
{
    flash_preinit(QSPI0);
...
#define SPLL_NEW_ANALOG_POWER_DISABLE                (0x44F01CA7)
#define SPLL_NEW_ANALOG_POWER_ENABLE_WITHOUT_M       (0x44F01CA4) 
#define SPLL_NEW_ANALOG_POWER_ENABLE_WITH_M          (0xC4F01CA4) 
#define SPLL_NEW_ANALOG_CONFIG1                      (0x775F5800)
#define SPLL_NEW_ANALOG_CONFIG2                      (0x07563425) 

#define AUPLL_NEW_ANALOG_POWER_DISABLE               (0x44F01CA7)
#define AUPLL_NEW_ANALOG_POWER_ENABLE_WITHOUT_M      (0x44F01CA4) 
#define AUPLL_NEW_ANALOG_POWER_ENABLE_WITH_M         (0xC4F01CA4) 
#define AUPLL_NEW_ANALOG_CONFIG1                     (0x775F5800)
#define AUPLL_NEW_ANALOG_CONFIG2                     (0x07563425) 
...
__RAM_CODE int System_SPLL_config(System_SPLLConfig_t *PLLConfig, uint32_t fu32_timeout)
{
    ...
    else
    {
        ...
        if (PLLConfig->PowerEn)
        {
            SYSTEM->SPLLConfig1 = SPLL_NEW_ANALOG_CONFIG1;
            SYSTEM->SPLLConfig2 = SPLL_NEW_ANALOG_CONFIG2;
            ...
            if (SYSTEM->SPLLConfig3.PLL_M)
            {
                SYSTEM->SPLLConfig0 = SPLL_NEW_ANALOG_POWER_ENABLE_WITH_M;
            }
            else
            {
                SYSTEM->SPLLConfig0 = SPLL_NEW_ANALOG_POWER_ENABLE_WITHOUT_M;
            }
            system_delay_us(50);
            __SYSTEM_SPLL_CLK_DIV2_ENABLE();
            System_SPLLCLK = (PLLConfig->PLL_N*HSE_VALUE) + mul_mod(PLLConfig->PLL_M,HSE_VALUE,0xffff);
        ...
        {
            SYSTEM->SPLLConfig0 = SPLL_NEW_ANALOG_POWER_DISABLE;
            System_SPLLCLK = 0;
        }    
    }
    return 0;
...
__RAM_CODE int System_AUPLL_config(System_AUPLLConfig_t *PLLConfig, uint32_t fu32_timeout)
{
    ...
    else
    {
        ...
        if (PLLConfig->PowerEn)
        {
            SYSTEM->AUPLLConfig1 = AUPLL_NEW_ANALOG_CONFIG1;
            SYSTEM->AUPLLConfig2 = AUPLL_NEW_ANALOG_CONFIG2;
            ...
            {
                SYSTEM->AUPLLConfig0 = AUPLL_NEW_ANALOG_POWER_ENABLE_WITH_M;
            }
            else
            {
                SYSTEM->AUPLLConfig0 = AUPLL_NEW_ANALOG_POWER_ENABLE_WITHOUT_M;
            }
            system_delay_us(50);
        ...
        {
            SYSTEM->AUPLLConfig0 = AUPLL_NEW_ANALOG_POWER_DISABLE;
            System_AUPLLCLK = 0;
        }
    }
    return 0;    
 ...
 * @fn      System_AUPLL_Inc
 *
 * @brief   Increase audio pll clk by step.
 */
void System_AUPLL_Inc(void)
{
    if((SYSTEM->AUPLLConfig3.PLL_M) && (SYSTEM->AUPLLConfig3.PLL_M < (0xffff - 4))){
        SYSTEM->AUPLLConfig3.PLL_M += 4; // d_sample = (20*24M/pll_d)/8/512, pll_d == 10000   
    }
}
/*********************************************************************
 * @fn      System_AUPLL_Dec
 *
 * @brief   Decrease audio pll clk by step.
 */
void System_AUPLL_Dec(void)
{
    if((SYSTEM->AUPLLConfig3.PLL_M) && (SYSTEM->AUPLLConfig3.PLL_M > 4)){
        SYSTEM->AUPLLConfig3.PLL_M -= 4;
    }
}
...
__RAM_CODE_FRONT static uint32_t frspim_rd_front(uint8_t addr, uint8_t len)
{
    uint32_t data;
    volatile struct frspim_reg_t * frspim_reg = (volatile struct frspim_reg_t * )FRSPIM_BASE;
    
    *(volatile uint32_t *)(&frspim_reg->ctrl) = 0x00001001 | (addr<<16) | (len << 4);   
...
__RAM_CODE_FRONT static void frspim_wr_front(uint8_t addr, uint8_t len, uint32_t val)
{
    volatile struct frspim_reg_t * frspim_reg = (volatile struct frspim_reg_t * )FRSPIM_BASE;

    *(volatile uint32_t *)(&frspim_reg->wdat) = val;
    *(volatile uint32_t *)(&frspim_reg->ctrl) = 0x00001101 | (addr<<16) | (len << 4);
    
    while(!frspim_reg->ctrl.done);
}

__RAM_CODE_FRONT void __power_on(void)
{
#define ool_write_front(addr, data)         frspim_wr_front((addr),1, (data))
#define ool_read_front(addr)                (uint8_t)frspim_rd_front((addr),1)

#define ool_write16_front(addr,data)        frspim_wr_front((addr),2, (data))
#define ool_read16_front(addr)              (uint16_t)frspim_rd_front((addr),2)

#define ool_write32_front(addr,data)        frspim_wr_front((addr),4, (data))
#define ool_read32_front(addr)              (uint32_t)frspim_rd_front((addr),4)
    ...
    ool_write_front(PMU_REG_CLK_EN, ool_read_front(PMU_REG_CLK_EN) | PMU_WDT_CLK_EN_BIT);
    /* set iwdt Count value */
    ool_write32_front(PMU_REG_WDT_LEN_0, 2000);
    /* set iwdt timeout value */
    ool_write16_front(PMU_REG_WDT_TOUT_COUNTER_0, 100);
    /* Set iwdt control */
    ool_write_front(PMU_REG_WDT_CTRL, PMU_WDT_RST_CHIP_BIT);
    /* enable iwdt count */
    ool_write_front(PMU_REG_WDT_CTRL, ool_read_front(PMU_REG_WDT_CTRL) | PMU_WDT_EN_BIT);
    
    while(1);
}

#ifdef __CC_ARM
__RAM_CODE_FRONT __ASM static void power_on(void)
{
   PRESERVE8
...
__RAM_CODE_FRONT static void power_on(void)
{
    /* initialize Stack */
    __asm (
        "CPSID   I\n"
...
__RAM_CODE __attribute__((noinline)) static void power_off_enter(void)
{
    uint32_t org_value;
    org_value = ool_read32(0xfc);
    if (org_value & 0x01) {
        ool_write32(0xfc/*PMU_REG_CPU_RESET_VECTOR*/, (uint32_t)power_on | 0x01);
    }
    else {
        ool_write32(0xfc/*PMU_REG_CPU_RESET_VECTOR*/, (uint32_t)power_on & (~0x01));
    }
 ...
 * @fn      system_power_off
 *
 * @brief   Put system into power off mode. User have to handle GPIO properly to avoid
 *          power leakage before calling this function.
 *          User should call this function before controller is initialized and all BT
 *          function is disabled.
 *          Procedure:
 *              1. call btdm_host_send_vendor_cmd(0x36, 0, NULL); to notify controller
 *                 prepare for power off
 *              2. do the following process in function btdm_host_vendor_cmd_cmp_evt
 *                      if (param[0] == 0x36) {
 ...
 *                          system_delay_us(100);
 *                          ool_write16(PMU_REG_PIN_PULL_EN, 0x03ff);
 *                          ool_write16(PMU_REG_PIN_PULL_SEL, 0x03fd);
 *                          system_power_off();
 *                      }
 */
__RAM_CODE __attribute__((noinline)) void system_power_off(void)
{    
    __disable_irq();

    pmu_ram_retention_set(PMU_RAM_RET_CRAM0_BIT);
...
/*********************************************************************
 * @fn      system_power_off_post_process
 *
 * @brief   This is an example showing how to handle power off command complete
 *          event received form controlelr side.
 *          user should implement this function in application layer.
 */
__WEAK void system_power_off_post_process(void)
{
    /* disable all interrupt */
    __disable_irq();
    
    /* user should add custom settings here */
    ...
    GPIOA->GPIO_BIT_SET = GPIO_PIN_3;
    SYSTEM->PortA_L_FuncMux &= (~(0xf<<12));
    /* wait for RTS is valid in controller side */
    system_delay_us(100);
    
    /* pull down wake up pin as default setting */
    ool_write16(PMU_REG_PIN_PULL_EN, ool_read16(PMU_REG_PIN_PULL_EN) | 0x0002);
    ool_write16(PMU_REG_PIN_PULL_SEL, ool_read16(PMU_REG_PIN_PULL_SEL) & (~0x0002));
...
#elif defined(CHIP_SEL_FR3066EQC)
    __SYSTEM_FR3066EQC_QSPI0_IO2_IO3_CONFIG();
#else
 ...
 *
 * @brief   system lvd protect hamdle.
 *
 */
__RAM_CODE void system_lvd_protect_handle(void)
{
    static uint8_t check_count = 0;
    GLOBAL_INT_DISABLE();
    while(1)
    {
        system_delay_us(100 * 500);
        if((ool_read(PMU_REG_ANA_STATUS) & PMU_LVD_STATUS_BIT) == 0)
        {
            check_count++;
            if(check_count >= 3)
                break;
 ...
 * @fn      system_dig_reset
 *
 * @brief   reset digital(core, peripheral).
 */
void system_dig_reset(void)
{
    __disable_irq();

    /* The system select the OSC24MHz before reset. */
    ...
    /* The uart select the OSC24MHz before reset. */
    __SYSTEM_UART_CLK_SELECT_COREH();

    /* The qspi select the OSC24MHz before reset. */
    System_XIPConfig_t XipConfig;
    XipConfig.CLK_SRC_SEL = XIP_CLK_SEL_COREH;
    XipConfig.DIV_SEL     = QSPI_BAUDRATE_DIV_4;
    XipConfig.RD_TYPE     = FLASH_RD_TYPE_QUAD;
    XipConfig.WR_TYPE     = FLASH_WR_TYPE_SINGLE;
    system_xip_flash_init(&XipConfig, true);
