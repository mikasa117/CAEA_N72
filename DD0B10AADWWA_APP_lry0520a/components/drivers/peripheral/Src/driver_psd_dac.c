...
void psd_dac_init(PSD_DAC_HandleTypeDef *hpsd)
{
    ...
    /* gain compensate */
    switch (hpsd->Init.Oversampling_Level)
    {
        case PSD_DAC_OVERSAMPLING_LOW:
        {
            if (__PSD_DAC_IS_USB_MODE())
            {
                if (hpsd->Init.SampleRate >= PSD_DAC_SAMPLE_RATE_11025)
                    __PSD_DAC_SET_GAIN_COMPENSATE(1978);
                else
                    __PSD_DAC_SET_GAIN_COMPENSATE(1492);
            }
            else{
                __PSD_DAC_SET_GAIN_COMPENSATE(1779);
            }
        }break;

        case PSD_DAC_OVERSAMPLING_MIDDLE:
        {
            if (__PSD_DAC_IS_USB_MODE())
            {
                if (hpsd->Init.SampleRate >= PSD_DAC_SAMPLE_RATE_11025)
                    __PSD_DAC_SET_GAIN_COMPENSATE(1984);
                else
                    __PSD_DAC_SET_GAIN_COMPENSATE(1498);
            }
            else{
                __PSD_DAC_SET_GAIN_COMPENSATE(1785);
            }
        }break;

        case PSD_DAC_OVERSAMPLING_HIGH:
        {
            if (__PSD_DAC_IS_USB_MODE())
            {
                if (hpsd->Init.SampleRate >= PSD_DAC_SAMPLE_RATE_11025)
                    __PSD_DAC_SET_GAIN_COMPENSATE(1867);
                else
                    __PSD_DAC_SET_GAIN_COMPENSATE(1916);
            }
            else{
                __PSD_DAC_SET_GAIN_COMPENSATE(1785);
            }
 ...
 * @param   fu16_Volume: volume dB = 20*lg(fu32_Volume/2^12)
 */
...
/*********************************************************************
 * @fn      psd_dac_set_volume_unit_dB
 *
 * @brief   set channel volume.
 * 
 * @param   fe_Channel: channel select.
 *          This parameter can be one of the following values:
 *          @arg PSD_DAC_CH_L:  set left channel volume.
 *          @arg PSD_DAC_CH_R:  set right channel volume.
 *          @arg PSD_DAC_CH_LR: set left and right channel volume.
 * @param   fs32_dB: fs32_dB: -72dB ~ 24dB. Default : 0dB.
 */
void psd_dac_set_volume_unit_dB(enum_PSD_DAC_Channel_t fe_Channel, double fs32_dB)
{
    if (fs32_dB > 24)
        fs32_dB = 24;
    if (fs32_dB < -72)
    ...
    uint32_t lu32_Volume = (uint32_t)(pow(10.0, ((double)fs32_dB - 0.3)/20.0) * 0x1000);

    if (fe_Channel == PSD_DAC_CH_L)
    {
        __PSD_DAC_SET_LEFT_VOLUME(lu32_Volume);
    }
    else if (fe_Channel == PSD_DAC_CH_R)
    {
        __PSD_DAC_SET_RIGHT_VOLUME(lu32_Volume);
    }
    else
    {
        __PSD_DAC_SET_LEFT_VOLUME(lu32_Volume);
