...
#define FLASH_ID_GD_GD25LQ16E           0x001560C8
...
static __RAM_CODE void flash_set_read_dtr_fast_quad(struct qspi_regs_t *qspi)
{
...
static __RAM_CODE void flash_set_read_fast_quad(struct qspi_regs_t *qspi)
{
...
static __RAM_CODE void flash_set_read_quad(struct qspi_regs_t *qspi)
{
...
static __RAM_CODE void flash_set_read_dtr_fast_dual(struct qspi_regs_t *qspi)
{
...
static __RAM_CODE void flash_set_read_fast_dual(struct qspi_regs_t *qspi)
{
...
static __RAM_CODE void flash_set_read_dual(struct qspi_regs_t *qspi)
{
...
static __RAM_CODE void flash_set_read_dtr_fast_single(struct qspi_regs_t *qspi)
{
...
static __RAM_CODE void flash_set_read_fast_single(struct qspi_regs_t *qspi)
{
...
static __RAM_CODE void flash_set_read_single(struct qspi_regs_t *qspi)
{
...
static __RAM_CODE void flash_set_write_quad(struct qspi_regs_t *qspi)
{
...
static __RAM_CODE void flash_set_write_dual(struct qspi_regs_t *qspi)
{
...
static __RAM_CODE void flash_set_write_single(struct qspi_regs_t *qspi)
{
 ...
 * @fn      flash_preinit
 *
 * @brief   flash pre-initialization, flash power-up protect(only xipflash).         
 *
 * @param   qspi: qspi controller base address
 */
__RAM_CODE void flash_preinit(struct qspi_regs_t *qspi)
{   
    ...
    if ((flash_id == FLASH_ID_PUYA_P25Q32)
            ...
            || (flash_id == FLASH_ID_GIANTEC_GT25Q80A)) {
        flash_protect_bit_set(qspi,0x1F,0x06,false,true);
    }
    else if ((flash_id == FLASH_ID_GD_GD25WQ80E)
            || (flash_id == FLASH_ID_PUYA_P25Q80)
            || (flash_id == FLASH_ID_PUYA_P25Q40) 
            || (flash_id == FLASH_ID_GD_GD25LQ16E)){
        flash_protect_bit_set(qspi,0x1F,0x06,false,false);
    }
    else {
        while(1);
    }
...
/************************************************************************************
 * @fn      flash_wait_wip_clear
 *
 * @brief   flash Wait wip to clear.
 *
 * @param   qspi: qspi controller base address
 */
...
/************************************************************************************
 * @fn      flash_enable_quad
 *
 * @brief   flash enable quad mode.
 *
 * @param   qspi: qspi controller base address
 */
 ...
 * @fn      flash_set_IO_DRV
 *
 * @brief   flash set IO_DRV(only GT).
 *
 * @param   qspi: qspi controller base address
 */
...
/************************************************************************************
 * @fn      flash_set_capture_delay
 *
 * @brief   flash set capture delay.
 *
 * @param   qspi: qspi controller base address.
 *           delay: capture delay value. 
 */
...
/************************************************************************************
 * @fn      flash_set_read_type
 *
 * @brief   flash set read type.
 *
 * @param   qspi: qspi controller base address.
 *          rd_type: read type. @ref flash_rd_type_t. 
 */
...
/************************************************************************************
 * @fn      flash_set_write_type
 *
 * @brief   flash set write type.
 *
 * @param   qspi: qspi controller base address.
 *          wr_type: write type. @ref flash_wr_type_t. 
 */
...
/************************************************************************************
 * @fn      flash_init_controller
 *
 * @brief   flash initializes the controller.
 *
 * @param   qspi: qspi controller base address.
 *          rd_type: read type. @ref flash_rd_type_t.
 *          wr_type: write type. @ref flash_wr_type_t. 
...
/************************************************************************************
 * @fn      flash_set_baudrate
 *
 * @brief   flash set baudrate.
 *
 * @param   qspi: qspi controller base address.
 *          baudrate:  set flash baudrate divider. @ref qspi_baud_rate_t
 */
 ...
 *
 * @param   qspi: qspi controller base address.
 *          read_high: Whether or not to read the flash high status register.
 */
 ...
 *          write_high: Whether the flash high status are written consecutively.
 */
...
/************************************************************************************
 * @fn      flash_write_status_2.
 *
 * @brief   flash write status2.
 *
 * @param   qspi: qspi controller base address.
 *          status: Write status2 value.
 */
...
/************************************************************************************
 * @fn      flash_write_status_volatile.
 *
 * @brief   flash write status volatile(cmd: 0x50).
 *          status: Write status value. 
 *          write_high: Whether the flash high status are written consecutively.
 */
...
/************************************************************************************
 * @fn      flash_write_status_2_volatile.
 *
 * @brief   flash write status2 volatile(cmd: 0x50).
 *
 * @param   qspi: qspi controller base address.
 *          status: Write status2 value.
 */
...
__RAM_CODE void flash_protect_bit_set(struct qspi_regs_t *qspi, uint8_t bits, uint8_t cmp, uint8_t wr_volatile, uint8_t status_2_separate)
{
    ...
    if(cmp != 0xff) {
        ...
        if(flash_id == FLASH_ID_GD_GD25LQ16E) { 
            status_tmp = flash_read_status(qspi, true);   
            status[0] = status_tmp & 0xff;            
            status[1] = (status_tmp>>8) & 0xff;            
        }
 ...
 *
 * @param   qspi: qspi controller base address.
 *          offset: flash operation address offset.
 *          length: buffer length to be write from flash section.
...
__RAM_CODE __attribute__((noinline)) uint8_t flash_write(struct qspi_regs_t *qspi, uint32_t offset, uint32_t length, const uint8_t *buffer)
{
    ...
    flash_protect_bit_set(QSPI0, 0x00, 0xFF, true, false);
    ...
    flash_protect_bit_set(QSPI0, 0x1f, 0xff, true, false);
    if ((uint32_t)qspi == FLASH_QSPI_BASE) {
 ...
 * @param   qspi: qspi controller base address.
 *          offset: flash operation address offset.
 *          length: buffer length to be read from flash section.
 *          buffer: pointer to buffer which will store data.
 */
 ...
 * @param   qspi: qspi controller base address.
 *          offset: flash operation address offset.
 *          size: erase the size of the flash.
 */
...
/************************************************************************************
 * @fn      flash_enter_deep_sleep.
 *
 * @brief   flash enter deep_sleep.
 *
 * @param   qspi: qspi controller base address.
 */
...
/************************************************************************************
 * @fn      flash_exit_deep_sleep.
 *
 * @brief   flash exit deep sleep.
 *
 * @param   qspi: qspi controller base address.
 */
...
* @fn      flash_OTP_read
*
* @brief   read data from OTP sections.
*
* @param   offset: Security flash section offset, only valid for 0x1000,0x2000,0x3000 
*          length: buffer length to be read from security flash section.
*          buffer: pointer to buffer which will store data
*
* @return  None.
*/
__RAM_CODE void flash_OTP_read(struct qspi_regs_t *qspi,uint32_t offset, uint32_t length, uint8_t *buffer)
{
    ...
    switch(flash_id)
    {
        case FLASH_ID_PUYA_P25Q32:
        case FLASH_ID_XMC_XM25LU32:
        case FLASH_ID_GD_GD25WQ80E:
        case FLASH_ID_GD_GD25LQ16E:
        {
            offset &= 0x3000;
        ...
        case FLASH_ID_PUYA_P25Q16:
        case FLASH_ID_PUYA_P25Q80: 
        case FLASH_ID_PUYA_P25Q40: 
        case FLASH_ID_XMC_XMQ25U80:
        {
            offset &= 0x3000;
        ...
        case FLASH_ID_GIANTEC_GT25Q16A:
        case FLASH_ID_GIANTEC_GT25Q80A:
        {
            offset &= 0x3000;
            offset -= 0x1000;
            offset >>= 4;
...
* @fn      flash_OTP_write
*
* @brief   write data to OTP sections.
*
* @param   offset: Security flash section offset, only valid for 0x1000,0x2000,0x3000 
*          length: buffer length to be written into security flash section.
...
*
* @param   offset :Security flash section offset.
*
* @return  None.
*/
__RAM_CODE void flash_OTP_erase(struct qspi_regs_t *qspi,uint32_t offset)
{
    ...
    switch(flash_id)
    {
        ...
        case FLASH_ID_GIANTEC_GT25Q80A:           
        {
            ...
            if(backup_offset == 0x1000)
            {
                flash_OTP_read(qspi, 0x2000, 256, otp_data);
                flash_OTP_read(qspi, 0x3000, 256, otp_data + 256);
            }
            else if(backup_offset == 0x2000)
            {
                flash_OTP_read(qspi, 0x1000, 256, otp_data);
                flash_OTP_read(qspi, 0x3000, 256, otp_data + 256);
            }
            else
            {
                flash_OTP_read(qspi, 0x1000, 256, otp_data);
                flash_OTP_read(qspi, 0x2000, 256, otp_data + 256);
            }
        } break;
    ...
    {    
        if(backup_offset == 0x1000)
        {
            flash_OTP_write(qspi, 0x2000, 256, otp_data);
            flash_OTP_write(qspi, 0x3000, 256, otp_data + 256);
        }
        else if(backup_offset == 0x2000)
        {
            flash_OTP_write(qspi, 0x1000, 256, otp_data);
            flash_OTP_write(qspi, 0x3000, 256, otp_data + 256);
        }
        else
        {
            flash_OTP_write(qspi, 0x1000, 256, otp_data);
            flash_OTP_write(qspi, 0x2000, 256, otp_data + 256);
        }
    }    
}
...
__RAM_CODE void flash_gen_uuid(struct qspi_regs_t *qspi, uint8_t *buffer, enum flash_gen_uuid_type_t type)
{
    ...
    if ((flash_id == FLASH_ID_GIANTEC_GT25Q16A)
        ...
        *(volatile uint32_t *)&qspi->read_conf = org_cfg;
        ...
        if (flash_id == FLASH_ID_PUYA_P25Q16) {
