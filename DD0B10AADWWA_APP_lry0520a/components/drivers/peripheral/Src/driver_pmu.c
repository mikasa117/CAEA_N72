...
#define PMU_PKSTPD_OFF_CNT      (0x03<<5)
...
{ 
    ...
    /* increase the supply voltage of the core and flash  */
    ool_write(PMU_REG_DSP_DLDO_CTRL, (ool_read(PMU_REG_DSP_DLDO_CTRL) & (~0x07)) | 0x05);   // 1.25v
    ool_write(PMU_REG_APP_DLDO_CTRL, (ool_read(PMU_REG_APP_DLDO_CTRL) & (~0x07)) | 0x05);   // 1.25v
    ool_write(PMU_REG_IOLDO2_CTRL_0, (ool_read(PMU_REG_IOLDO2_CTRL_0)&0xF0) | 0x06);    // 1.9v
...
//    ool_write16(PMU_REG_PIN_INPUT_EN, 0x0000);
    ...
    ool_write(PMU_REG_PLLLDO_CTRL_0, 0xA2);
    
    /* sw configure iodldo,buck,dldo voltage enable */
    ool_write(PMU_REG_POW_SW_CFG_EN0, 0x03);
    ool_write(PMU_REG_POW_SW_CFG_EN1, 0x30);
...
/******************************************************************************
 * @fn      pmu_ext_xtal32K_control
 *
 * @brief   external xtal 32.765K control.
 *
 * @param   en:  true: enable, false: disable.
 * @param   cap: Matching capacitance.
 */
void pmu_ext_xtal32K_control(bool en, enum_PMU_XTAL32K_CAP_t cap)
{
    uint8_t reg = ool_read(PMU_REG_XTAL_CTRL);

    if (en)
    {
        ool_write(PMU_REG_XTAL_CTRL, (reg & ~(PMU_XTAL_CAP_BIT | PMU_XTAL_EN_BIT)) | (cap << PMU_XTAL_CAP_POS));
    }
    else
    {
        ool_write(PMU_REG_XTAL_CTRL, reg | PMU_XTAL_EN_BIT);
    }
}
 ...
 *
 * @brief   pmu clock source select.
 * 
 * @param   pmu_clock_sel: pmu clock source select.
 */
void pmu_low_speed_xtal_config(enum_PMU_clock_sel_t pmu_clock_sel)
{
    if (pmu_clock_sel == PMU_CLK_SEL_RCLP_DIV2)
    {
        /* Select the internal RC clock as pmu clock source */
        ool_write(PMU_REG_CLK, (ool_read(PMU_REG_CLK) & ~PMU_SYSCLK_SEL_BIT));
    }
