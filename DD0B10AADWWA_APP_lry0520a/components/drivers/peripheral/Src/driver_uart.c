...
__WEAK void uart_IRQHandler(UART_HandleTypeDef *huart)
{
    switch (__UART_INT_GET_ID(huart->UARTx))
    {
        case INT_INDEX_TXE: 
        {
            ...
            {                
                if(__UART_IS_TxFIFO_EMPTY(huart->UARTx))
                {
                    GLOBAL_INT_DISABLE();
        ...
        default: 
        {
            volatile REG_USR_t usr_status = huart->UARTx->USR;
        }break; 
    }
}
...
/************************************************************************************
 * @fn      uart_transmit_IT_stop
 *
 * @brief   uart transmit IT stop.
 *
 * @param   huart: UART handle.
 */
void uart_transmit_IT_stop(UART_HandleTypeDef *huart)
{
    huart->b_TxBusy = false; 
...
/************************************************************************************
 * @fn      uart_receive_IT_stop
 *
 * @brief   uart receive IT stop.
 *
 * @param   huart: UART handle.
 */
void uart_receive_IT_stop(UART_HandleTypeDef *huart)
{
    huart->b_RxBusy = false; 
