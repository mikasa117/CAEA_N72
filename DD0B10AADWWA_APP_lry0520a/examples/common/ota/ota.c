 ...
 */
static uint8_t app_otas_crc_cal(uint32_t firmware_length,uint32_t new_bin_addr,uint32_t crc_data_t);
...
uint32_t app_otas_get_storage_address(void)
{
    ...
    uint32_t start_address = (512+8)*1024;
    a_image_tail = a_info->image_length + a_info->execute_offset;
    //flash 4K对齐
    if((a_image_tail > start_address))
    {
        start_address = (a_image_tail + 0xfff) & 0x0ffff000;
    }
