...
|    Macro Definition
|******************************************************************************/
#define SYSM_TASK_PERIOD            10u  //10ms

#define SYSM_TIME_INIT2RUN          (500u / SYSM_TASK_PERIOD)           // 500ms
#define SYSM_TIME_PRESLEEP_LIMIT    (500u / SYSM_TASK_PERIOD)           // 500ms
#define SYSM_TIME_PRESLEEP_TIMEOUT  (60000u / SYSM_TASK_PERIOD)         // 60s
#define SYSM_TIME_SLEEP             (20u / SYSM_TASK_PERIOD)            // 200ms

#define SYSM_TIME_MCU_TIMEOUT       (500u/SYSM_TASK_PERIOD)  //500s

#define SYSM_MCU_RESET()            NVIC_SystemReset()

#define SYSM_EN_SOURCE              SYSM_EN_TYPE_AD
#define SYSM_EN_TYPE_AD             0u
#define SYSM_EN_TYPE_IO             1u

#define SYSM_NET_CONTROL_EN         FALSE
...
#define TEST_SOFTWARE_FUNC_EXHIBITION         TEST_SOFTWARE_FUNC_DISABLE
    ...
    #define SYSM_NET_SLEEP()         TRUE //GetCanApp_u_NetWorkSleepAllowed()
#endif

#define SYSM_EN_INVALID_FILTCNT     (200u/SYSM_TASK_PERIOD)  //200ms
#define SYSM_EN_VALID_FILTCNT       (10u/SYSM_TASK_PERIOD)  //10ms

#if (SYSM_EN_SOURCE == SYSM_EN_TYPE_AD)
    #define SYSM_GET_EN_ADC()           adc_if_get_validValue(adc_cl_acc)
    #define SYSM_EN_AD_VALID            (uint16)990u  	//6.5V--1000u
    #define SYSM_EN_AD_INVALID          (uint16)620u    //4V--615u
#elif (SYSM_EN_SOURCE == SYSM_EN_TYPE_IO)
    #define SYSM_GET_EN_STS()           TRUE
    #define SYSM_EN_IO_VALID            (uint8)TRUE
    #define SYSM_EN_IO_INVALID          (uint8)FALSE
#else
...
|    Typedef Definition
|******************************************************************************/
typedef uint8_t (* tSysM_FlagGetFunc)(void);
...
extern void SysM_Set_SysPowerStatus(uint8_t e_u_Status);
extern uint8_t SysM_Get_SysPowerStatus(void);
extern uint8_t SysM_GetSysSTA(void);
#endif 
/*EOF*/
