...
void PINS_Init(const pin_settings_config_t * config)
{
    ...
    regValue |= PCTRL_PCR_PFE(config->passiveFilter ? 1U : 0U);  /*PRQA S 2985*/
#endif
    ...
    regValue |= PCTRL_PCR_LOCK(config->pinLock ? 1U : 0U);  /*PRQA S 2985*/
#endif
    ...
    if (PCTRL_MUX_AS_GPIO == config->mux)
    {
        ...
        /* Configure direction */
        config->gpioBase->POER = GPIO_POER_POE(directions);  /*PRQA S 2985*/
    }
    
    /* config irq register */
    config->gpioBase->PCR[config->pinPortIdx] &= ~(GPIO_PCR_IRQC_MASK);
    config->gpioBase->PCR[config->pinPortIdx] |= GPIO_PCR_IRQC(config->intConfig);  /*PRQA S 2985*/
    if (config->clearIntFlag)
    {
        config->gpioBase->PIFR = ((uint32_t)0x01) << config->pinPortIdx;
    }
}
