                      ...
                      uint32_t chnCount)
{
    ...
    for (index = 0U; index < DMA_INSTANCE_COUNT; index++)
#else
    index = 0U;
#endif
    {
    ...
    /* Check if it is necessary to enable ERRATA E406002 */
#if defined(FEATURE_DMA_ERRATA_E406002) && (FEATURE_DMA_ERRATA_E406002 == 1)
    if (dmaStatus == STATUS_SUCCESS)
    {
        if (userConfig->maxChannelForChLink == true)
        {
            DEV_ASSERT(chnCount < FEATURE_DMA_VIRTUAL_CHANNELS);
            const dma_channel_config_t dma_dummy_config = {
            ...
            chnInitStatus = DMA_DRV_ChannelInit(&dma_dummyconfig_State,&dma_dummy_config);
            if (chnInitStatus == STATUS_SUCCESS)
            {
                dmaStatus = DMA_DRV_ConfigSingleBlockTransfer(FEATURE_DMA_VIRTUAL_CHANNELS-1U, DMA_TRANSFER_PERIPH2PERIPH, (uint32_t)&CIM->DIEINFO, (uint32_t)&CIM->DIEINFO, DMA_TRANSFER_SIZE_4B, 4);
                DMA_DRV_ConfigureInterrupt(FEATURE_DMA_VIRTUAL_CHANNELS-1U, DMA_CHN_MAJOR_LOOP_INT, false);
                (void)DMA_DRV_StartChannel(FEATURE_DMA_VIRTUAL_CHANNELS-1U);
            }
        }
        else
        {
            /* No need for the final channel to assist in DMA transmission. */
        }
    }
#endif
...
status_t DMA_DRV_Deinit(void)
{
...
#if (FEATURE_DMA_VIRTUAL_ERROR_INTERRUPT_LINES > 1U)
    for (index = 0U; index < FEATURE_DMA_VIRTUAL_ERROR_INTERRUPT_LINES; index++)
#endif
    {
    ...
    if (s_virtEdmaState != NULL)
    {
        ...
        for (index = 0U; index < FEATURE_DMA_VIRTUAL_CHANNELS_INTERRUPT_LINES; index++)
        {
                             ...
                             const dma_channel_config_t *dmaChannelConfig)
{
    ...
    irqNumber = s_dmaIrqId[virtualChannel];
...
#ifdef FEATURE_DMAMUX_AVAILABLE
    (void)DMA_DRV_SetChannelRequestAndTrigger(dmaChannelConfig->virtChnConfig,
...
void DMA_DRV_IRQHandler(uint8_t virtualChannel)
{
    ...
    if (DMA_DRV_GetIntStatus(virtualChannel))
    {
...
void DMA_DRV_ECCErrorIRQHandler(uint8_t dmaInstance)
{
    ...
    if ((dmaRegBase->SECCES & DMA_SECCES_SECCEF_MASK) != 0U)
    {
        ...
        dmaReg = dmaRegBase->EIE;
        dmaReg &= ~(DMA_EIE_EIEN_MASK);
        dmaRegBase->EIE = dmaReg | DMA_EIE_TAG(0x5A);
    }
}
#endif /* FEATURE_DMA_SUPPORT_ECC_ERROR_CHECK */
/*FUNCTION**********************************************************************
                                         ...
                                         uint8_t ctsCount)
{
    ...
    for (i = 0U; (i < ctsCount) && (retStatus == STATUS_SUCCESS); i++)
    {
        ...
        if (i == ((uint8_t) (ctsCount - 1U)))
        {
            ...
            dmaTransferConfig.ramReloadEnable = false;
        } else
        {
...
void DMA_DRV_CancelTransfer(bool error)
{
    ...
    for (dmaInstance = 0U; dmaInstance < DMA_INSTANCE_COUNT; dmaInstance++)
#endif
    {
                              ...
                              dma_software_cts_t *scts)
{
    if ((config != NULL) && (scts != NULL))
    {
        ...
        scts->ATTR = (uint16_t) (DMA_CTS_TCR_SMOD(config->srcModulo) | DMA_CTS_TCR_SSIZE(config->srcTransferSize) |
                                 DMA_CTS_TCR_DMOD(config->destModulo) | DMA_CTS_TCR_DSIZE(config->destTransferSize)); /* PRQA S 2985 */
...
dma_chn_status_t DMA_DRV_GetChannelStatus(uint8_t virtualChannel)
{
    ...
    volatile dma_chn_state_t *chnState = s_virtEdmaState->virtChnState[virtualChannel];
