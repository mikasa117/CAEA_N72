...
static void FLEXCAN_ClearRAM(CAN_Type * base)
{
    ...
    for (databyte = 0U; databyte < (4U * maxMbNum); databyte++) {
        base->RAM[databyte] = 0x0U;
    }
    for (databyte = 0U; databyte < maxMbNum; databyte++) {
        base->RXIMR[databyte] = 0x0U;
    }
#if FEATURE_CAN_HAS_RAM_ECC
    /* Clear RX Individual Mask Regions for RXIMRn */
    RAM = (volatile uint32_t *)((uint32_t)base + (uint32_t)0x880);
    for (databyte = 0U; databyte < maxMbNum; databyte++) {
        RAM[databyte] = 0x0U;
    }
    /* Clear RXFIR,RXMGMASK,RX14MASK,RX15MASK,RXFGMASK Regions*/
    RAM = (volatile uint32_t *)((uint32_t)base + (uint32_t)0xa80);
    for (databyte = 0U; databyte < 12U; databyte++) {
        RAM[databyte] = 0x0U;
    }
    /* Clean SMB of Classical CAN */
    RAM = (volatile uint32_t *)((uint32_t)base + (uint32_t)0xab0);
    for (databyte = 0U; databyte < 12U; databyte++) {
        RAM[databyte] = 0x0U;
    }
    /* Clear SMB region in CANFD mode */
    RAM = (volatile uint32_t *)((uint32_t)base + (uint32_t)0xf28);
    ...
    if (FLEXCAN_HasHRTIMESTAMP(base))
    {
        ...
        RAM = (volatile uint32_t *)((uint32_t)base + (uint32_t)0xc20);
        RAM[0] = 0x0U;
        RAM = (volatile uint32_t *)((uint32_t)base + (uint32_t)0xc24);
        RAM[0] = 0x0U;
        /* Clear High-resolution Time Stamp Regions for CAN_HR_TIME_STAMPn */
        RAM = (volatile uint32_t *)((uint32_t)base + (uint32_t)0xc30);
        for (databyte = 0U; databyte < maxMbNum; databyte++) {
            RAM[databyte] = 0x0U;
        }
    }
    ...
    /* Clear enhance fifo and enhance fifo filter table if target has enhance fifo feature and ECC on */
    if(FLEXCAN_HasEnhanceRxFIFO(base))
    {
        ...
        for (databyte = 0U; databyte < (32U * FEATURE_CAN_ENHANCE_RX_FIFO_COUNT); databyte++) {
            RAM[databyte] = 0x0U;
        }
...
{
    uint32_t MaskNum = 0UL;
    uint32_t MaxMBNum = 0UL;
    ...
    /* Get instanceX max MB */
    MaxMBNum = FLEXCAN_GetMaxMbNum(base);

    /* Set all RXIMR register 0xFFFFFFFF */
    for(MaskNum = 0U; MaskNum < MaxMBNum; MaskNum++)
    {
        (base->RXIMR[MaskNum]) = CAN_RXIMR_MI_MASK;
    }
