...
static const IRQn_Type g_flexcanOredMessageBufferIrqId[FEATURE_CAN_MB_IRQS_MAX_COUNT][CAN_INSTANCE_COUNT] = FEATURE_CAN_MB_IRQS;

/* Pointer to runtime state structure.*/
static flexcan_state_t * g_flexcanStatePtr[CAN_INSTANCE_COUNT] = FEATURE_FLEXCAN_STATE_PTR;
...
static status_t FLEXCAN_StartRxMessageEnhanceFifoData(
                    uint8_t instance,
                    flexcan_msgbuff_t *data,
                    bool isBlocking
                    );
    ...
    uint32_t mask)
{
    ...
    if (((uint32_t)mb_idx > FLEXCAN_GetMaxMsgBuffNum(base)) || (mb_idx >= (uint8_t)FEATURE_CAN_RXIMR_COUNT))
    {
        if (freeze == false)
        {
            FLEXCAN_ExitFreezeMode(base);
        ...
        ReturnStatus = STATUS_CAN_BUFF_OUT_OF_RANGE;
    }
    else
    {
   ...
   const flexcan_user_config_t *data)
{
...
#endif /* FEATURE_CAN_HAS_PE_CLKSRC_SELECT */
    ...
    {   
        ...
        /* Enable RxFIFO feature, if requested. 
        * This might fail if the FD mode is enabled without the Enhance Rx FIFO feature. */
        if (data->is_rx_fifo_needed)
        {
#if FEATURE_CAN_HAS_ENHANCE_RX_FIFO
            /* Enable enhance rx fifo if enabled */
            if(FLEXCAN_InstanceHasEnhanceRxFIFO(instance) && data->is_enhance_rx_fifo_needed)
            {
                state->enhanceRxFifoState.state = FLEXCAN_MB_IDLE;
                state->enhanceRxFifoState.isBlocking = false;
                state->enhanceRxFifoState.mb_message = NULL;

                if((data->num_enhance_rx_fifo_filters > 0U) && (data->num_enhance_rx_fifo_filters >= data->num_enhance_rx_fifo_extid_filters))
                {               
                    /* Enable enhance rx fifo,  set the filter num and extended Id filter number */
                    result = FLEXCAN_EnableEnhanceRxFifo(base, data->num_enhance_rx_fifo_filters - 1U, data->num_enhance_rx_fifo_extid_filters);
                }
                else
                {
                    result = STATUS_ERROR;
                ...
                {
                    if(data->num_enhance_rx_fifo_min_messages > 0U)
                    {
                        /* Set enhance fifo water mark */
                        result = FLEXCAN_SetEnhanceRxFifoWatermark(base, data->num_enhance_rx_fifo_min_messages - 1U);
                    }
                    else
                    {
                        result = STATUS_ERROR;
    ...
    if (result != STATUS_SUCCESS)  /* PRQA S 2992 */  /* PRQA S 2996 */
    {
        result = STATUS_ERROR;
    }   
    ...
    if(result == STATUS_SUCCESS)
    {
        ...
        if (data->transfer_type == FLEXCAN_RXFIFO_USING_DMA)
        {
            ...
            if(FLEXCAN_InstanceHasEnhanceRxFIFO(instance) && data->is_enhance_rx_fifo_needed)  
            {
                if(FLEXCAN_IsEnhanceRxFifoEnabled(base))
                {
                    FLEXCAN_SetRxFifoDMA(base, true);
                    FLEXCAN_SetEnhanceRxFifoDMALastWord(base, data->rxFifoDMALastWord);
                }
                else
                {
                    result = STATUS_ERROR;
    ...
    uint32_t msg_id)
{
    ...
    else
    {
        ...
        result = FLEXCAN_SetTxMsgBuff(base, mb_idx, &cs, msg_id, NULL, false);
    }
    return result;
}
/*FUNCTION**********************************************************************
    ...
    const uint8_t *mb_data)
{
    ...
    else
    {
        ...
        result =  FLEXCAN_SetTxMsgBuff(base, mb_idx, &cs, msg_id, mb_data, tx_info->is_remote);
    }
    ...
    uint32_t timeout_ms)
{
    ...
    if(result != STATUS_CAN_BUFF_OUT_OF_RANGE)
    {
        ...
        if (result == STATUS_SUCCESS)
        {
            ...
            if (status == STATUS_TIMEOUT)
            {
                ...
                else
                {
                    ...
                    (void) OSIF_SemaWait(&state->mbs[mb_idx].mbSema, 0);
                }
            }
        }
    }
    ...
    uint8_t id_filter_table_length)
{
    ...
    /* Initialize rx fifo*/
    FLEXCAN_SetEnchanceRxFifoFilter(base, id_filter_table,id_filter_table_length);
    ...
    uint32_t timeout_ms)
{
    ...
    if (result == STATUS_SUCCESS)
    {
        ...
        else
        {
            ...
            (void)DMA_DRV_StopChannel(state->rxFifoDMAChannel);
        }
#endif
        ...
        if (result == STATUS_TIMEOUT)
        {
            ...
            if (state->mbs[FLEXCAN_MB_HANDLE_RXFIFO].state == FLEXCAN_MB_IDLE)
            {
                ...
                (void) OSIF_SemaWait(&state->mbs[FLEXCAN_MB_HANDLE_RXFIFO].mbSema, 0);
            }
            else
            {
    ...
    flexcan_msgbuff_t *data)
{
    ...
    result = FLEXCAN_StartRxMessageEnhanceFifoData(instance, data, false);
...
uint8_t FLEXCAN_DRV_GetTDCValue(uint8_t instance)
{
    ...
    result = (uint8_t)((base->ETDC & CAN_ETDC_ETDCVAL_MASK) >> CAN_ETDC_ETDCVAL_SHIFT);
#else
    result = (uint8_t)((base->FDCTRL & CAN_FDCTRL_TDCVAL_MASK) >> CAN_FDCTRL_TDCVAL_SHIFT);
#endif
    return result;
}
...
bool FLEXCAN_DRV_GetTDCFail(uint8_t instance)
{
    ...
    const CAN_Type * base = g_flexcanBase[instance];
#if defined (CAN_CTRL2_BTE_MASK)
    result = (((base->ETDC & CAN_ETDC_ETDCFAIL_MASK) >> CAN_ETDC_ETDCFAIL_SHIFT) != 0u);
#else
    result =  (((base->FDCTRL & CAN_FDCTRL_TDCFAIL_MASK) >> CAN_FDCTRL_TDCFAIL_SHIFT) != 0U);
#endif
    return result;
}
...
void FLEXCAN_DRV_ClearTDCFail(uint8_t instance)
{
    ...
    CAN_Type * base = g_flexcanBase[instance];
#if defined (CAN_CTRL2_BTE_MASK)
    base->ETDC = base->ETDC | CAN_ETDC_ETDCFAIL_MASK;
#else
...
static void FLEXCAN_IRQHandlerRxMB(uint8_t instance, uint32_t mb_idx)
{
    ...
    if ((FLEXCAN_IsRxFifoEnabled(base)) && (((state->mbs[mb_idx].mb_message->cs & CAN_CS_CODE_MASK) >> CAN_CS_CODE_SHIFT) == (uint32_t)FLEXCAN_RX_INACTIVE))
    {
        ...
        *flexcan_mb |= (((uint32_t)FLEXCAN_RX_EMPTY) << CAN_CS_CODE_SHIFT) & CAN_CS_CODE_MASK;
    }
    else
    {
...
void FLEXCAN_ORed_IRQHandler(uint8_t instance)
{
    ...
    if (state->error_callback != NULL)
    {
        if ((base->ESR1 & CAN_ESR1_BOFFDONEINT_MASK)!= (uint32_t)0)
        {
            state->error_callback(instance, FLEXCAN_BUS_OFF_DONE_EVENT, state);
        }
        if ((base->ESR1 & CAN_ESR1_TXWRN_MASK)!= (uint32_t)0)
        {
            state->error_callback(instance, FLEXCAN_TX_WARNING_EVENT, state);
        }
        if ((base->ESR1 & CAN_ESR1_RXWRN_MASK)!= (uint32_t)0)
        {
            state->error_callback(instance, FLEXCAN_RX_WARNING_EVENT, state);
        }
        if ((base->ESR1 & CAN_ESR1_BOFFINT_MASK)!= (uint32_t)0)
        {
            state->error_callback(instance, FLEXCAN_BUS_OFF_ENTER_EVENT, state);
        }
    }
...
void FLEXCAN_IRQHandler(uint8_t instance, uint8_t mb_idx_start, uint8_t mb_idx_end)  /* PRQA S 4700 */
{
    ...
    flexcan_state_t * state = g_flexcanStatePtr[instance];
#if FEATURE_CAN_HAS_ENHANCE_RX_FIFO
    bool instanceHasEnhanceRxFIFO = false;
    bool isEnhanceRxFifoEnabled = false;
#endif /* FEATURE_CAN_HAS_ENHANCE_RX_FIFO */
    /* Get the interrupts that are enabled and ready */
    uint32_t mb_idx;
    for (mb_idx = mb_idx_start; mb_idx <=mb_idx_end; mb_idx++)
    {
        flag_reg = FLEXCAN_GetMsgBuffIntStatusFlag(base, mb_idx);
        if((flag_reg & (uint32_t)1)== (uint32_t)1)
        {
            break;
        }
    }
    ...
    {
#if FEATURE_CAN_HAS_ENHANCE_RX_FIFO
        instanceHasEnhanceRxFIFO = FLEXCAN_InstanceHasEnhanceRxFIFO(instance);
        if(instanceHasEnhanceRxFIFO)
        {
            isEnhanceRxFifoEnabled = FLEXCAN_IsEnhanceRxFifoEnabled(base);

            /* Check enhance fifo interrups if instance has enhance fifo feature and enabled */
            if(isEnhanceRxFifoEnabled)
            {
                uint8_t enhanceRxFifoAvailbale = FLEXCAN_GetEnhanceRxFIFODataAvailableStatusFlag(base);
                uint8_t enhanceRxFifoOverflow = FLEXCAN_GetEnhanceRxFIFOOverflowStatusFlag(base);
                uint8_t enhanceRxFifoWatermark = FLEXCAN_GetEnhanceRxFIFOWatermarkIndStatusFlag(base);
                if ((enhanceRxFifoOverflow != 0U) || (enhanceRxFifoWatermark != 0U) || (enhanceRxFifoAvailbale != 0U))
                {
                    FLEXCAN_EnhanceRxFifo_IRQHandler(instance);
                }
            }
        }
#endif /* FEATURE_CAN_HAS_ENHANCE_RX_FIFO */
         ...
         * only on FLEXCAN Instance 0 - inly for YTM32B1L */
        if (instance == 0U)
        {
...
#if FEATURE_CAN_HAS_ENHANCE_RX_FIFO
void FLEXCAN_EnhanceRxFifo_IRQHandler(uint8_t instance)
{
    DEV_ASSERT(instance < CAN_INSTANCE_COUNT);
    ...
    if((FLEXCAN_GetEnhanceRxFIFODataAvailableStatusFlag(base)) != 0U)
    {
        if(state->enhanceRxFifoState.state == FLEXCAN_MB_RX_BUSY)
        {
            FLEXCAN_ReadEnhanceRxFifo(base, state->enhanceRxFifoState.mb_message);
            FLEXCAN_ClearEnhanceRxFIFODataAvailableStatusFlag(base);
            ...
            if (state->enhanceRxFifoState.state == FLEXCAN_MB_IDLE)
            {
                /* Complete receive data */
                FLEXCAN_CompleteRxMessageEnhanceFifoData(instance);
            }       
        }
    }
    if((FLEXCAN_GetEnhanceRxFIFOOverflowStatusFlag(base)) != 0U)
    {
        FLEXCAN_ClearEnhanceRxFIFOOverflowStatusFlag(base);
        /* Invoke callback */
        if (state->callback != NULL)
        {
            state->callback(instance,
    ...
    if((FLEXCAN_GetEnhanceRxFIFOWatermarkIndStatusFlag(base)) != 0U)
    {
        FLEXCAN_ClearEnhanceRxFIFOWatermarkIndStatusFlag(base);
        /* Invoke callback */
        if (state->callback != NULL)
        {
            state->callback(instance,
...
void FLEXCAN_Error_IRQHandler(uint8_t instance)
{
    ...
    if (state->error_callback != NULL)
    {
        if ((base->ESR1 & CAN_ESR1_ERROVR_MASK)!= (uint32_t)0)
        {
            state->error_callback(instance, FLEXCAN_ERROR_OVERRUN_EVENT, state);
        }
#if FEATURE_CAN_HAS_FD
        if ((base->ESR1 & CAN_ESR1_ERRINT_FAST_MASK)!= (uint32_t)0)
        {
            state->error_callback(instance, FLEXCAN_FD_DATA_BIT_ERROR_EVENT, state);
        }
#endif
...
#endif
        if ((base->ESR1 & CAN_ESR1_ERRINT_MASK)!= (uint32_t)0)
        {
            state->error_callback(instance, FLEXCAN_BIT_ERROR_EVENT, state);
        }
        if ((base->ESR1 & CAN_ESR1_WAKINT_MASK)!= (uint32_t)0)
        {
            state->error_callback(instance, FLEXCAN_WAKEUP_EVENT, state);
        }
#if FEATURE_CAN_HAS_RAM_ECC
        if ((base->ERRSR & (CAN_ERRSR_FANCEIF_MASK | CAN_ERRSR_HANCEIF_MASK))!= (uint32_t)0)
        {
            state->error_callback(instance, FLEXCAN_RAM_ECC_ERROR_EVENT, state);
        }
#endif
    }
...
#elif FEATURE_CAN_HAS_SELF_WAKE_UP
    CAN_Type * base = g_flexcanBase[instance];
    flexcan_state_t * state = g_flexcanStatePtr[instance];
...
status_t FLEXCAN_DRV_GetTransferStatus(uint8_t instance, uint8_t mb_idx)
{
    ...
    const flexcan_state_t * state = g_flexcanStatePtr[instance];
    status_t status;
    flexcan_mb_state_t mb_state;

#if FEATURE_CAN_HAS_ENHANCE_RX_FIFO
    if(mb_idx == FLEXCAN_MB_HANDLE_ENHANCE_RXFIFO)
    {
        mb_state = state->enhanceRxFifoState.state;
    }
    else
#endif /* FEATURE_CAN_HAS_ENHANCE_RX_FIFO */
    {
        mb_state = state->mbs[mb_idx].state;
    }

    if (mb_state == FLEXCAN_MB_IDLE)
    {
        status = STATUS_SUCCESS;
    }
#if FEATURE_CAN_HAS_DMA_ENABLE
    else if (mb_state == FLEXCAN_MB_DMA_ERROR)
    {
        status = STATUS_ERROR;
    }
#endif
                    ...
                    )
{
    ...
    else if (state->mbs[mb_idx].state != FLEXCAN_MB_IDLE)
    {
        result = STATUS_BUSY;
    }
    else
                    ...
                    )
{
    ...
    else if(!FLEXCAN_IsRxFifoEnabled(base))/* Check if RxFIFO feature is enabled */
    {
        result = STATUS_ERROR;
    }
    else
    {
        ...
        if (state->transferType == FLEXCAN_RXFIFO_USING_DMA)
        {
            ...
            dmaStatus = DMA_DRV_InstallCallback(state->rxFifoDMAChannel,
                                                FLEXCAN_CompleteRxFifoDataDMA,
                                                (void *)((uint32_t)instance));/* PRQA S 0326 */
            ...
            {
                dmaStatus = DMA_DRV_ConfigSingleBlockTransfer(state->rxFifoDMAChannel,
                ...
                {
                    DMA_DRV_DisableRequestsOnTransferComplete(state->rxFifoDMAChannel, true);

                    dmaStat = DMA_DRV_StartChannel(state->rxFifoDMAChannel);
                    if (dmaStat != STATUS_SUCCESS)
                    {
                        state->mbs[FLEXCAN_MB_HANDLE_RXFIFO].state = FLEXCAN_MB_IDLE;
                        result = STATUS_ERROR;
                    }
                    ...
                    )
{
    ...
    else if (!FLEXCAN_IsEnhanceRxFifoEnabled(base))
    {
        result = STATUS_ERROR;
    }
    else
    {
        state->enhanceRxFifoState.state = FLEXCAN_MB_RX_BUSY;
        state->enhanceRxFifoState.isBlocking = isBlocking;
        state->enhanceRxFifoState.mb_message = data;
        /* Enable enhance RX FIFO interrupts */
        if (state->transferType == FLEXCAN_RXFIFO_USING_INTERRUPTS)
        {
            FLEXCAN_SetEnhanceRxFIFOOverflowINT(base,true);
            FLEXCAN_SetEnhanceRxFIFOWatermarkIndINT(base,true);
            FLEXCAN_SetEnhanceRxFIFODataAvailableINT(base,true);
            result = STATUS_SUCCESS;
        }
        else
        {
...
static void FLEXCAN_CompleteRxMessageFifoData(uint8_t instance)
{
    ...
    else
    {
        if (state->mbs[FLEXCAN_MB_HANDLE_RXFIFO].state != FLEXCAN_MB_DMA_ERROR)
        {
            ...
            uint32_t *msgData_32 = (uint32_t *)fifo_message->data;/* PRQA S 0310 */ /* PRQA S 3305 */
...
static void FLEXCAN_CompleteRxMessageEnhanceFifoData(uint8_t instance)
{
    ...
    if (state->transferType == FLEXCAN_RXFIFO_USING_INTERRUPTS)
    {
        /* Disable ehnhanc RX FIFO interrupts */
        FLEXCAN_SetEnhanceRxFIFOOverflowINT(base,false);
        FLEXCAN_SetEnhanceRxFIFOWatermarkIndINT(base,false);
        FLEXCAN_SetEnhanceRxFIFODataAvailableINT(base,false);
    } 
    state->enhanceRxFifoState.mb_message = NULL;

    /* Update status for receive by using fifo*/
    if (state->enhanceRxFifoState.isBlocking == true)
    {
        if (state->enhanceRxFifoState.state == FLEXCAN_MB_IDLE)
        {
            status_t status = OSIF_SemaPost(&state->enhanceRxFifoState.mbSema);
            DEV_ASSERT(status == STATUS_SUCCESS);
            (void)status;
        }
    }
...
static inline uint32_t FLEXCAN_CheckDsample(uint32_t tmpSample, uint32_t samplePoint)
{
    ...
    {
        result = (tmpSample - samplePoint);
    }
    else
    {
        result = (samplePoint - tmpSample);
    }
    return result;
}
...
static inline uint32_t FLEXCAN_CheckdBitrate(uint32_t tmpBitrate, uint32_t bitrate)
{
    ...
    {
        result = (tmpBitrate - bitrate);
    }
    else
    {
        result = (bitrate - tmpBitrate);        
    }
    return result;
}
    ...
    for (tmpPresdiv = 0U; tmpPresdiv < FLEXCAN_PRESDIV_MAX; tmpPresdiv++) {
        ...
        if ((numTq >= FLEXCAN_NUM_TQ_MIN) && (numTq < FLEXCAN_NUM_TQ_MAX))
        {
            ...
            if (((tSeg1 >= FLEXCAN_TSEG1_MAX) || (tSeg2 >= FLEXCAN_TSEG2_MAX) || (tSeg2 < FLEXCAN_TSEG2_MIN) || (tSeg1 < FLEXCAN_TSEG1_MIN)) ||
               ((tmpPropseg >= FLEXCAN_PROPSEG_MAX) || (tmpPseg1 >= FLEXCAN_PSEG1_MAX) || (tmpPseg2 < FLEXCAN_PSEG2_MIN) || (tmpPseg2 >= FLEXCAN_PSEG2_MAX))) /*PRQA S 2996*/
            {
                continue;
            }
