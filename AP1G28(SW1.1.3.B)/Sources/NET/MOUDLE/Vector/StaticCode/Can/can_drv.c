...
/* PRQA S 6010 EOF */ /* No check of HIS rules for static path count */
/* PRQA S 6030 EOF */ /* No check of HIS rules for cyclomatic complexity */
/* PRQA S 6050 EOF */ /* No check of HIS rules for number of distinct function calls */
/* PRQA S 6080 EOF */ /* No check of HIS rules for nesting of control structures */
...
#if defined( C_ENABLE_RX_BASICCAN_OBJECTS )
/* CODE CATEGORY 1 START */
static void CanBasicCanMsgReceived(CAN_HW_CHANNEL_CANTYPE_FIRST CanObjectHandle rxMailboxHandle, CanObjectHandle rxObjHandle);
/* CODE CATEGORY 1 END */
...
/* CODE CATEGORY 1 START */
static void CanHL_TxConfirmation(CAN_CHANNEL_CANTYPE_FIRST CanObjectHandle txMailboxHandle, CanObjectHandle txObjHandle);
/* CODE CATEGORY 1 END */
# if defined( C_ENABLE_TX_POLLING )
/* CODE CATEGORY 1 START */
...
#  endif
  ...
  /* check for transmit message object free --------------------------------- */
    if (   ( canHandleCurTxObj[logTxObjHandle] != kCanBufferFree )    /* MsgObj used?  */
        || ( !CanLL_TxIsObjFree(canHwChannel, Can_Mailbox[txMailboxHandle].HwObjHandle) )
...
************************************************************************** */
/* CODE CATEGORY 1 START */
static void CanHL_TxConfirmation( CAN_HW_CHANNEL_CANTYPE_FIRST CanObjectHandle txMailboxHandle, CanObjectHandle txObjHandle)
{
  CanObjectHandle       logTxObjHandle; /* PRQA S 0781 */ /* MD_Can_0781 */
  CanTransmitHandle     txHandle;
...
V_DEF_FUNC(CAN_LOCAL_INLINE, vuint8, STATIC_CODE) CanLL_InitBeginSetRegisters( CAN_CHANNEL_AND_HW_CHANNEL_CANTYPE_FIRST CanInitInfoStructPtr localInfo )
{
  ...
  tmpCANmcr = CAN_READ_PROTECTED_REG(pFlexCANLocal->canmcr) & kNotFlexCAN_MCR_CFG_BTIS;
...
V_DEF_FUNC(CAN_LOCAL_INLINE, vuint8, STATIC_CODE) CanLL_InitMailboxTx( CAN_CHANNEL_AND_HW_CHANNEL_CANTYPE_FIRST CanInitInfoStructPtr localInfo )
{
  ...
  {
    LOCAL_MAILBOX_ACCESS(canHwChannel, localInfo->hwObjHandle, control) = kTxCodeInactive; /* set transmit object inactive */
  }
...
# else /* C_ENABLE_CLASSIC_MAILBOX_LAYOUT */
  for(filterHandle = 0; filterHandle < NUMBER_OF_FILTERS(canHwChannel); filterHandle++)
  {
    if (filterHandle < NUMBER_OF_FULL_CONFIGURABLE_FILTERS(canHwChannel))  /* PRQA S 3355,3358 */ /* MD_Can_13.7,MD_Can_13. */
    {
      ...
      pIndivRxMask->indivRxMask[filterHandle] = (vuint32)((locMsgIDMask << 1) | kRxFIFO_MASK); /* bits REM and EXT must always match */
    }
    ...
    /* if Extended Id */
    if((Can_GetInitCodeOfInitBasicCan(Can_GetInitBasicCanIndex(localInfo->initObject) + filterHandle) & kExtIDBit) != 0)
    {
...
#  if !defined ( C_ENABLE_CLASSIC_MAILBOX_LAYOUT )
  ...
  pFlexCANLocal->rxfgmask = (kRxFIFO_MASK | (vuint32)0x3FFFFFFE); /* bits REM and EXT must always match */
#  endif
# endif
...
# endif
  tmpControlVal = (vuint16)(((vuint16)localInfo->dlcRaw & kTxDlcMask) | kTxCodeFree); /* write IDE,SRR and DLC */
...
V_DEF_FUNC(CAN_LOCAL_INLINE, void, STATIC_CODE) CanLL_CanInterruptDisable(CAN_HW_CHANNEL_CANTYPE_FIRST tCanLLCanIntOldPtr localInterruptOldFlagPtr)
{
  ...
  CAN_DISABLE_BUSOFF_INTERRUPT(localInterruptOldFlagPtr);
...
V_DEF_FUNC(CAN_LOCAL_INLINE, void, STATIC_CODE) CanLL_CanInterruptRestore(CAN_HW_CHANNEL_CANTYPE_FIRST tCanLLCanIntOld localInterruptOldFlag)
{
  ...
  CAN_RESTORE_BUSOFF_INTERRUPT(localInterruptOldFlag);
...
V_DEF_FUNC(CAN_LOCAL_INLINE, vuint8, STATIC_CODE) CanLL_SetFlexCANToInitMode(CAN_HW_CHANNEL_CANTYPE_ONLY)
{
    ...
    || ((CAN_READ_PROTECTED_REG_32BIT(pFlexCANLocal->estat) & kFlexCAN_FCS_BOFF) == 0))
  {
    ...
    {
      CAN_WRITE_PROTECTED_REG_RESET_32BIT(pFlexCANLocal->control1, kFlexCAN_CLK_SRC); /* PRQA S 0277 */ /* MD_Can_0277_negation */
    }
...
MD_Can_0291_LL:
  Reason: An integer expression with a value that is apparently negative is being converted to an unsigned type.
  Risk: None
  Prevention: Code review and runtime tests
*/
