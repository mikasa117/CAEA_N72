 ...
 *  Daniel Hof                    visdhf        Vector Informatik GmbH
 ...
 *  4.23.00   2014-02-25  vissrk  ESCAN00073880 Added support for Immediate Msg transmission on RepeatMessageRequest
 *                                ESCAN00073913 Added support for CanNm_TxConfirmation forwarding
 *  4.24.00   2016-01-18  vismdr  ESCAN00083394 Added dummy statement in CanNm_Transmit
 *  4.25.00   2017-03-29  vissrk  ESCAN00094383 'Immediate Nm Transmission' shall also be executed in case of a passive startup
 *                                ESCAN00094536 CanNm shall support 'Msg Timeout Timer' greater than 'Msg Cycle Time'
 *  4.26.00   2017-06-14  vissrk  ESCAN00095546 Support Communication Control Extension
 *********************************************************************************************************************/
...
/* CANNM200: vendor specific version information check */
    ...
    || (CANNM_SW_MINOR_VERSION != (0x26u)) \
    || (CANNM_SW_PATCH_VERSION != (0x00u)) )
# error "Vendor specific version numbers of CanNm.c and CanNm.h are inconsistent"
#endif

/* CANNM200: vendor specific version information check */
#if (  (CANNM_CBK_MAJOR_VERSION != (0x04u)) \
    || (CANNM_CBK_MINOR_VERSION != (0x26u)) \
    || (CANNM_CBK_PATCH_VERSION != (0x00u)) )
...
# else
    if( CanNm_LConfigPtr->CANNM_MSG_TIMEOUT_TIME >= CanNm_LConfigPtr->CANNM_MSG_CYCLE_TIME )
    {
...
# endif
# if ( CANNM_COM_CONTROL_EXT_ENABLED == STD_ON )
...
# if ( CANNM_COM_CONTROL_EXT_ENABLED == STD_ON )
          if( CanNm_NmState[CANNM_CHANNEL_IDX] == NM_STATE_REPEAT_MESSAGE )
          {
          ...
          /* Trigger Passive Startup to enter Repeat Message state from any state */
...
#  if ( CANNM_FORWARD_TX_CONFIRMATION == STD_ON )
          /* ESCAN00073913 */
          Appl_CanNm_TxConfirmation( CANNM_NMTOSYSTEM_CHANNEL_IND );
#  endif
# endif
        }
      }
...
# if ( CANNM_IMMEDIATE_NM_TX_FEATURE_ENABLED == STD_ON )
        if( (CanNm_LConfigPtr->CANNM_IMMEDIATE_NM_TRANSMISSIONS > 0u) &&
            (
#  if ( CANNM_IMMEDIATE_NM_TX_ON_PASSIVE_START == STD_ON )
              /* (NetworkRequest or Passive startup) or (RxIndication in Prepare Bus Sleep) */
              ( (CanNm_NetworkRestartFlag[CANNM_CHANNEL_IDX] != CANNM_NO_RESTART) || (lastNmState == NM_STATE_PREPARE_BUS_SLEEP) )
#  else
              /* ESCAN00094383 */
              /* Network Request */
              (CanNm_NetworkRestartFlag[CANNM_CHANNEL_IDX] == CANNM_ACTIVE_RESTART) 
              && (1U == CanApp_GetImmediateNmTxFlag())  //The demand is not clear as to whether an NM message should be sent immediately.
#  endif
#  if ( CANNM_IMMEDIATE_TRANSMISSION_ON_REPEAT_MSG_REQUEST == STD_ON ) && ( CANNM_NODE_DETECTION_ENABLED == STD_ON )
              /* ESCAN00073880 */
              /* Repeat Message Request in Network Mode */
...
# endif
        {
          /* Repeat Message Indication received */
...
# if ( CANNM_COM_CONTROL_EXT_ENABLED == STD_ON )
      ( CanNm_TxControlState[CANNM_CHANNEL_IDX] == MSG_TX_ENABLED ) && 
# endif
      ( ( CanNm_NmState[CANNM_CHANNEL_IDX] == NM_STATE_READY_SLEEP ) || ( CanNm_NmState[CANNM_CHANNEL_IDX] == NM_STATE_NORMAL_OPERATION ) )
    )
  {
...
# else
#  if ( CANNM_USE_DUMMY_STATEMENT == STD_ON )
  /* PRQA S 3112 2 */ /* MD_CanNm_3112 */ /* ESCAN00083394 */
  (void)CanNmTxPduId;
  (void)PduInfoPtr;
#  endif
...
#endif
#if ( defined ( CANNM_COM_CONTROL_EXT_ENABLED ) && ( ( CANNM_COM_CONTROL_EXT_ENABLED == STD_OFF ) || ( CANNM_COM_CONTROL_EXT_ENABLED == STD_ON ) ) )
#else
# error: "CAN NM configuration issue: Communication Control Extension Enabled has to be defined STD_OFF or STD_ON"
#endif
...
#endif
#if ( defined (CANNM_IMMEDIATE_TRANSMISSION_ON_REPEAT_MSG_REQUEST ) && ( ( CANNM_IMMEDIATE_TRANSMISSION_ON_REPEAT_MSG_REQUEST == STD_OFF ) || ( CANNM_IMMEDIATE_TRANSMISSION_ON_REPEAT_MSG_REQUEST == STD_ON ) ) )
#else
# error "CAN NM configuration issue: Immediate Transmission on Repeat Message Request has to be defined STD_OFF or STD_ON"
#endif
#if ( defined (CANNM_FORWARD_TX_CONFIRMATION ) && ( ( CANNM_FORWARD_TX_CONFIRMATION == STD_OFF ) || ( CANNM_FORWARD_TX_CONFIRMATION == STD_ON ) ) )
#else
# error "CAN NM configuration issue: Forward Tx Confirmation has to be defined STD_OFF or STD_ON"
#endif
...
#if ( CANNM_COM_CONTROL_ENABLED == STD_OFF ) && ( CANNM_COM_CONTROL_EXT_ENABLED == STD_ON )
# error: "CAN NM configuration issue: CANNM_COM_CONTROL_EXT_ENABLED cannot be activated if CANNM_COM_CONTROL_ENABLED is STD_OFF."
#endif
